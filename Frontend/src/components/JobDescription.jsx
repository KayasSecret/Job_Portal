...
const JobDescription = ({ job }) => {
  ...
  const applyJobHandler = async () => {
    try {
      const res = await axios.get(`${APPLICANT_API_ENDPOINT}/apply/${jobId}`, { withCredentials: true })
      console.log(res.data)
      if (res.data.success) {
        setIsApplied(true) // Update the local state
        const updateSingleJob = { ...singleJob, applications: [...singleJob.applications, { applicant: user?._id }] }
        dispatch(setSingleJob(updateSingleJob)) // The will update, and changes to the UI will be visible.
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <Badge className={"text-[#F83002] font-bold "} variant="ghost">Type : {singleJob?.jobType}</Badge>
